...
//         <button className="ivCtrlBtn" onClick={handleZoomOut} disabled={scale <= 1} aria-label="Zoom out"><FiZoomOut /></button>
//         <span className="ivZoomText">{Math.round(scale * 100)}%</span>
//         <button className="ivCtrlBtn" onClick={handleZoomIn} disabled={scale >= 4} aria-label="Zoom in"><FiZoomIn /></button>
//       </div>
//       {currentIndex > 0 && (
//         <button className="ivNavBtn ivPrev" onClick={handlePrevImage} aria-label="Previous image"><ChevronLeft size={24} /></button>
//       )}
//       {currentIndex < images.length - 1 && (
//         <button className="ivNavBtn ivNext" onClick={handleNextImage} aria-label="Next image"><ChevronRight size={24} /></button>
//       )}
...
//     const dateStr = toYMD(date);
//     return (checkInDate && dateStr === toYMD(checkInDate)) || (checkOutDate && dateStr === toYMD(checkOutDate));
//   };

//   const isDateInRange = (date) => {
//     if (!date || !checkInDate || !checkOutDate) return false;
//     const dateStr = toYMD(date);
//     return dateStr > toYMD(checkInDate) && dateStr < toYMD(checkOutDate);
//   };
...
//         {daysOfWeek.map(day => (
//           <div key={day} style={{ textAlign: 'center', fontSize: '0.75rem', fontWeight: '600', color: '#666', padding: '8px 0' }}>{day}</div>
//         ))}
//         {days.map((date, index) => (
...
//         <div><span style={{ fontSize: '0.85rem', color: '#666' }}>Check-in: </span><span style={{ fontWeight: '600' }}>{checkInDate ? checkInDate.toLocaleDateString() : 'Not selected'}</span></div>
//         <div><span style={{ fontSize: '0.85rem', color: '#666' }}>Check-out: </span><span style={{ fontWeight: '600' }}>{checkOutDate ? checkOutDate.toLocaleDateString() : 'Not selected'}</span></div>
//       </div>
//     </div>
//   );
// };
...
//   return (
//     <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', zIndex: 10002, display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(5px)' }}>
//       <div style={{ background: 'white', padding: '2.5rem', borderRadius: '16px', width: '90%', maxWidth: '450px', position: 'relative', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)' }}>
//         <button onClick={onClose} style={{ position: 'absolute', top: '15px', right: '15px', background: '#f1f1f1', border: 'none', borderRadius: '50%', width: '32px', height: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.2rem', cursor: 'pointer' }}><FiX color="#333" /></button>
...
//             Fill in your details below and our team will get back to you for <span style={{ fontWeight: '600', color: '#8b0000' }}>{propertyName}</span>.
//           </p>
//         </div>
...
//             <input type="text" required value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="Enter your name" style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid #e5e7eb', fontSize: '1rem', outline: 'none' }} onFocus={(e) => e.target.style.borderColor = '#8b0000'} onBlur={(e) => e.target.style.borderColor = '#e5e7eb'} />
//           </div>
//           <div>
//             <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: '600', color: '#374151', marginBottom: '0.5rem' }}>Email Address</label>
//             <input type="email" required value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} placeholder="Enter your email" style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid #e5e7eb', fontSize: '1rem', outline: 'none' }} onFocus={(e) => e.target.style.borderColor = '#8b0000'} onBlur={(e) => e.target.style.borderColor = '#e5e7eb'} />
//           </div>
//           <div>
//             <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: '600', color: '#374151', marginBottom: '0.5rem' }}>Phone Number</label>
//             <input type="tel" required value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} placeholder="Enter your phone number" style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid #e5e7eb', fontSize: '1rem', outline: 'none' }} onFocus={(e) => e.target.style.borderColor = '#8b0000'} onBlur={(e) => e.target.style.borderColor = '#e5e7eb'} />
//           </div>
//           <button type="submit" disabled={loading} style={{ marginTop: '1rem', width: '100%', padding: '14px', background: 'linear-gradient(135deg, #8b0000 0%, #a50000 100%)', color: 'white', border: 'none', borderRadius: '8px', fontWeight: 'bold', fontSize: '1rem', cursor: loading ? 'not-allowed' : 'pointer' }}>
//             {loading ? <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}><Loader size={18} className="animate-spin" /> Sending...</span> : 'Request Callback'}
//           </button>
//         </form>
//       </div>
//     </div>
...
//   // rentalType from list page: 'short' = nightly, 'long' = monthly
//   const [rentalType, setRentalType] = useState(() => {
//     return sessionStorage.getItem('ovika_rental_type') || 'short';
//   });
...
//           // Set pricing mode based on rentalType from list page
//           const storedRentalType = sessionStorage.getItem('ovika_rental_type') || 'short';
//           if (storedRentalType === 'long') {
...
//     const isMonthlyMode = pricingMode === 'monthly'; // applies to ALL property types
...
//             || Number(property.meta?.perMonthPrice)
//             || Number(property.meta?.monthlyPrice)
...
//         } else {
//           // Non-PG nightly
//           const price = selectedPrice || Number(property.meta?.perNightPrice) || Number(property.price) || 0;
...
//         if (currentDays <= 15) { nextTierDays = 16; nextTierPercentage = 15; daysNeededForNextTier = nextTierDays - currentDays; }
//         else if (currentDays <= 30) { nextTierDays = 31; nextTierPercentage = 30; daysNeededForNextTier = nextTierDays - currentDays; }
//       }
//     }
...
//       setPricing({ subtotal, discount: discountAmount, discountPercentage, gst: gstAmount, total: computedTotal, daysNeededForNextTier, nextTierPercentage });
//     } else {
...
//       return;
//     }
//     // Pick price based on current pricing mode
//     let roomPrice;
...
//     // If monthly mode, open lead modal instead of payment flow
//     if (pricingMode === 'monthly') {
...
//     // Monthly mode (long stay) - open lead modal for enquiry
//     if (pricingMode === 'monthly') {
...
//   const handleFileDrop = (e) => { e.preventDefault(); e.stopPropagation(); const files = e.dataTransfer.files; if (files.length > 0) handleFile(files[0]); };
//   const handleFileChange = (e) => { const files = e.target.files; if (files.length > 0) handleFile(files[0]); };
...
//       try {
//         const response = await fetch('https://www.townmanor.ai/api/image/aws-upload-owner-images', { method: 'POST', body: formDataLocal });
...
//     if (/^[0-1]{12}$/.test(aadhaarNumber) || /^(\d)\1{11}$/.test(aadhaarNumber)) { showAlert('The Aadhaar number appears to be invalid.'); return; }
...
//       if (response.data && response.data.success) { setFormData(prev => ({ ...prev, aadhaarVerified: true })); showAlert('Aadhaar verified successfully!'); }
//       else { showAlert(`Aadhaar verification failed: ${response.data?.message || 'Verification failed'}`); }
//     } catch (error) {
//       console.error('Aadhaar verification error:', error);
//       showAlert(`Aadhaar verification failed: ${error.response?.data?.message || error.message}`);
//     } finally {
...
//     if (file) { setPassportFile(file); setPassportError(''); }
//   };

//   const handleVerifyPassport = async () => {
//     setFormData(prev => ({ ...prev, passportVerified: false }));
//     setPassportError('');
//     if (!passportFile) { showAlert('Please upload an image or scan of the international passport to verify.'); return; }
...
//         setFormData(prev => ({ ...prev, passportVerified: true, verificationData: response.data.data || response.data }));
...
//       }
//     } catch (error) {
//       let errorMessage = 'Failed to verify passport.';
//       if (error.response) errorMessage = error.response.data?.message || `Server error (${error.response.status})`;
//       else if (error.request) errorMessage = 'No response from verification service.';
...
//     if (!clientId) { showAlert('Client ID missing. Please request OTP again.'); return; }
...
//       if (response.data && response.data.success) { setFormData(prev => ({ ...prev, mobileVerified: true })); showAlert('Mobile number verified successfully!'); }
//       else throw new Error(response.data?.message || 'OTP verification failed');
//     } catch (error) {
//       showAlert(`Error: ${error.response?.data?.message || error.message || 'Verification failed'}`);
//     } finally {
...
//     try {
//       if (!(formData.aadhaarVerified || formData.passportVerified)) { showAlert('Please verify your Aadhaar or Passport before proceeding.'); setIsSubmitting(false); return; }
//       let userLocal = {};
...
//     if (!bookingIdParam) { showAlert('Booking ID missing. Cannot proceed to payment.'); setIsSubmitting(false); return; }
//     try {
...
//             <button onClick={() => navigate('/')} style={{ width: '100%', padding: '12px', background: '#8b0000', color: 'white', border: 'none', borderRadius: '6px', fontSize: '1rem', fontWeight: '600', cursor: 'pointer' }}>Go to Home</button>
//           </div>
//         </div>
//       )}
...
//                       <div style={{ width: '30px', height: '30px', borderRadius: '50%', background: index + 1 <= step ? '#8b0000' : '#e5e5e5', color: index + 1 <= step ? 'white' : '#999', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 0.5rem', fontSize: '0.85rem', fontWeight: '600' }}>{index + 1}</div>
//                       <span style={{ fontSize: '0.75rem', fontWeight: index + 1 === step ? '600' : 'normal', color: index + 1 === step ? '#8b0000' : '#666', display: 'block' }}>{stepName}</span>
//                     </div>
//                   </div>
//                 ))}
//               </div>
...
//                     <p style={{ marginBottom: '1rem' }}>Guests are responsible for any damage caused to the property during their stay.</p>
...
//                           ⏳ Request sent. Please wait for owner approval.
//                         </div>
//                       )}
//                       <Calendar
...
//                         onDateSelect={async (dates) => { setFormData({ ...formData, ...dates }); }}
//                         minDate={new Date()}
...
//                             <span>Stay Discount ({pricing.discountPercentage}%)</span><span>-<MdCurrencyRupee style={{ display: 'inline' }} />{pricing.discount.toFixed(2)}</span>
//                           </div>
//                         )}
//                         <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.75rem', paddingBottom: '0.75rem', borderBottom: '1px solid #e5e5e5' }}>
//                           <span>Taxes & GST (5%)</span><span><MdCurrencyRupee style={{ display: 'inline' }} />{pricing.gst.toFixed(2)}</span>
//                         </div>
//                         <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: '600', fontSize: '1.2rem', color: '#8b0000' }}>
//                           <span>Total Price</span><span><MdCurrencyRupee style={{ display: 'inline' }} />{pricing.total.toFixed(2)}</span>
//                         </div>
//                         {pricing.discount > 0 && <div style={{ marginTop: '0.5rem', textAlign: 'right', color: '#16a34a', fontSize: '0.85rem', fontWeight: '600' }}>You saved ₹{pricing.discount.toFixed(2)} on this stay!</div>}
//                         {pricing.daysNeededForNextTier > 0 && pricing.daysNeededForNextTier <= 5 && (
//                           <div style={{ marginTop: '1rem', padding: '10px', background: '#eff6ff', borderRadius: '6px', border: '1px dashed #3b82f6', fontSize: '0.85rem', color: '#1e40af', textAlign: 'center' }}>
//                             💡 Add <strong>{pricing.daysNeededForNextTier} more {pricing.daysNeededForNextTier === 1 ? 'day' : 'days'}</strong> to get <strong>{pricing.nextTierPercentage}% discount!</strong>
//                           </div>
//                         )}
//                       </div>
//                     </div>
...
//                         <input type="text" inputMode="numeric" maxLength={12} value={aadhaarNumber} onChange={(e) => setAadhaarNumber(e.target.value.replace(/\D/g, ''))} placeholder="12-digit Aadhaar number" disabled={formData.aadhaarVerified} style={{ width: '100%', padding: '14px', borderRadius: '6px', border: '1px solid #ddd', fontSize: '1rem', background: formData.aadhaarVerified ? '#e8f5e9' : 'white' }} />
//                       </div>
//                       <button onClick={handleVerifyAadhaar} disabled={formData.aadhaarVerified || !aadhaarNumber || isAadhaarLoading} style={{ width: '100%', padding: '14px', background: formData.aadhaarVerified ? '#22c55e' : '#8b0000', color: 'white', border: 'none', borderRadius: '6px', cursor: formData.aadhaarVerified || !aadhaarNumber || isAadhaarLoading ? 'not-allowed' : 'pointer', fontSize: '1rem', fontWeight: '600', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
...
//                         <input type="file" accept="image/*,application/pdf" onChange={handlePassportFileSelect} disabled={formData.passportVerified || isPassportLoading} style={{ width: '100%', padding: '14px', borderRadius: '6px', border: '1px solid #ddd', background: formData.passportVerified ? '#e8f5e9' : 'white', cursor: formData.passportVerified || isPassportLoading ? 'not-allowed' : 'pointer' }} />
//                         {passportFile && <p style={{ marginTop: '0.5rem', fontSize: '0.9rem', color: '#666' }}><strong>Selected:</strong> {passportFile.name} ({Math.round(passportFile.size / 1024)} KB)</p>}
//                       </div>
//                       <button onClick={handleVerifyPassport} disabled={formData.passportVerified || !passportFile || isPassportLoading} style={{ width: '100%', padding: '14px', background: formData.passportVerified ? '#22c55e' : '#8b0000', color: 'white', border: 'none', borderRadius: '6px', cursor: formData.passportVerified || !passportFile || isPassportLoading ? 'not-allowed' : 'pointer', fontSize: '1rem', fontWeight: '600', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
...
//                   <div style={{ padding: '2rem', background: '#f8fafc', borderRadius: '8px' }}>
//                     <h3 style={{ marginBottom: '1rem', fontSize: '1.2rem', display: 'flex', alignItems: 'center', gap: '8px' }}>📱 Mobile Number Verification</h3>
//                     {!otpSent ? (
//                       <div style={{ marginBottom: '1rem' }}>
//                         <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: '500' }}>Enter Mobile Number</label>
//                         <div style={{ display: 'flex', gap: '10px' }}>
//                           <input type="text" inputMode="numeric" maxLength={10} value={mobileNumber} onChange={(e) => setMobileNumber(e.target.value.replace(/\D/g, ''))} placeholder="10-digit mobile number" disabled={formData.mobileVerified || isOtpLoading} style={{ flex: 1, padding: '14px', borderRadius: '6px', border: '1px solid #ddd', fontSize: '1rem', background: formData.mobileVerified ? '#e8f5e9' : 'white' }} />
//                           <button onClick={handleGenerateOTP} disabled={formData.mobileVerified || !mobileNumber || mobileNumber.length !== 10 || isOtpLoading} style={{ padding: '0 20px', background: '#8b0000', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: '600' }}>
...
//                         <div style={{ display: 'flex', gap: '10px' }}>
//                           <input type="text" inputMode="numeric" maxLength={6} value={otpInput} onChange={(e) => setOtpInput(e.target.value.replace(/\D/g, ''))} placeholder="Enter OTP" disabled={formData.mobileVerified || isMobileVerifying} style={{ flex: 1, padding: '14px', borderRadius: '6px', border: '1px solid #ddd', fontSize: '1rem', background: formData.mobileVerified ? '#e8f5e9' : 'white' }} />
//                           <button onClick={handleVerifyOTP} disabled={formData.mobileVerified || !otpInput || isMobileVerifying} style={{ padding: '0 20px', background: '#22c55e', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: '600' }}>
...
//                     {isPhotoUploading ? (
//                       <div><Loader size={56} className="animate-spin" style={{ margin: '0 auto 1rem', color: '#8b0000' }} /><p style={{ fontSize: '1.1rem', color: '#666' }}>Uploading your photo...</p></div>
//                     ) : (
//                       <><UploadCloud size={56} style={{ margin: '0 auto 1rem', color: '#8b0000' }} /><p style={{ fontSize: '1.1rem', marginBottom: '0.5rem' }}>Drag and drop your photo here</p><p style={{ color: '#666' }}>or click to browse</p></>
//                     )}
//                   </div>
...
//                         <div style={{ position: 'absolute', top: '10px', right: '10px', background: '#22c55e', color: 'white', padding: '8px', borderRadius: '50%' }}><CheckCircle size={24} /></div>
//                       </div>
//                     </div>
//                   )}
//                 </div>
...
//                     </div>
//                     <button onClick={handlePayNow} disabled={!isPayNowEnabled || isSubmitting} style={{ width: '100%', padding: '18px', background: isPayNowEnabled && !isSubmitting ? '#8b0000' : '#ccc', color: 'white', border: 'none', borderRadius: '8px', fontSize: '1.2rem', cursor: isPayNowEnabled && !isSubmitting ? 'pointer' : 'not-allowed', fontWeight: '700', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.75rem' }}>
//                       {isSubmitting ? <><Loader size={24} className="animate-spin" /> Processing Payment...</> : <>Pay Now <FiShield size={24} /></>}
//                     </button>
//                     {!isPayNowEnabled && <p style={{ marginTop: '1rem', color: '#ef4444', fontSize: '0.9rem' }}>Please complete all previous steps to enable payment</p>}
//                     <p style={{ marginTop: '1.5rem', fontSize: '0.85rem', color: '#666' }}><FiShield style={{ display: 'inline', verticalAlign: 'middle' }} /> Secure payment powered by PayU</p>
//                   </div>
//                 </div>
//               )}
//             </div>
...
//               <button onClick={handlePrev} disabled={step === 1} style={{ padding: '12px 32px', background: step === 1 ? '#eee' : '#f8fafc', border: '2px solid #ddd', borderRadius: '8px', cursor: step === 1 ? 'not-allowed' : 'pointer', fontSize: '1rem', fontWeight: '600', color: step === 1 ? '#999' : '#333' }}>← Previous</button>
//               {step === 3 && bookingType === 1 && bookingRequestStatus !== 'accepted' ? (
//                 <button onClick={() => sendAvailabilityRequest({ checkInDate: formData.checkInDate, checkOutDate: formData.checkOutDate })} disabled={!formData.checkInDate || !formData.checkOutDate || ownerApprovalStatus === 'pending'} style={{ padding: '12px 32px', background: ownerApprovalStatus === 'pending' ? '#ccc' : '#8b0000', color: 'white', border: 'none', borderRadius: '8px', fontSize: '1rem', fontWeight: '600', cursor: ownerApprovalStatus === 'pending' ? 'not-allowed' : 'pointer' }}>
//                   {ownerApprovalStatus === 'pending' ? "Request Sent" : "Send Booking Request"}
//                 </button>
//               ) : (
//                 <button onClick={handleNext} style={{ padding: '12px 32px', background: '#8b0000', color: 'white', border: 'none', borderRadius: '8px', fontSize: '1rem', fontWeight: '600', cursor: 'pointer' }}>Next →</button>
//               )}
//             </div>
//           </div>
//         </div>
...
//             <div className="feature-box"><BiBed className="f-icon"/><div><strong>{getDisplayCount(property.bedrooms, property.parsedBedrooms)}</strong><span>{property.property_category === 'PG' ? 'Room Type' : 'Bedroom'}</span></div></div>
...
//           {/* ================================================================
//               NON-PG ROOM ARRANGEMENTS TABLE — WITH BOOK NOW BUTTON + BED ICON
//               ================================================================ */}
...
//                         const price = Number(room.price) || Number(property.price) || 0;
//                         const isLast = i === property.parsedBedrooms.length - 1 && !property.parsedBathrooms?.length;
//                         return (
//                           <tr key={`bed-${i}`} className={`rm-row ${isLast ? 'rm-row--last' : ''}`}>
//                             <td className="rm-td rm-td--room">
//                               <div className="rm-room-cell">
//                                 {/* Bed Icon — small square like sketch */}
//                                 <div className="rm-bed-icon-wrap">
//                                   <div className="rm-bed-icon">
//                                     <div className="rm-bed-headboard"></div>
//                                     <div className="rm-bed-body">
//                                       <div className="rm-bed-pillow"></div>
//                                       <div className="rm-bed-pillow"></div>
//                                     </div>
//                                   </div>
//                                 </div>
...
//                             </td>
//                             <td className="rm-td">
//                               <span className="rm-area-val">{room.areaSqFt ? `${room.areaSqFt} sqft` : (property.area ? `${property.area} sqft` : '—')}</span>
//                             </td>
...
//                                 // Monthly price: property-level se lo (same jo booking card dikhata hai)
...
//                               <button className="rm-book-btn" onClick={() => handleRoomBookNow(room)}>
//                                 Book Now
//                               </button>
//                             </td>
//                           </tr>
...
//                           <td className="rm-td rm-td--room">
//                             <div className="rm-room-cell">
//                               <div className="rm-bed-icon-wrap" style={{ fontSize: '18px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>🛁</div>
//                               <div className="rm-room-info"><span className="rm-room-name">{bath.type || 'Bathroom'}</span></div>
//                             </div>
//                           </td>
//                           <td className="rm-td"><span className="rm-bath rm-bath--shared"><span className="rm-bath-dot"></span>Bathroom</span></td>
//                           <td className="rm-td"><span className="rm-area-val">—</span></td>
//                           <td className="rm-td rm-td--price"><span className="rm-on-request">Included</span></td>
//                           <td className="rm-td"><div className="rm-avail rm-avail--now"><span className="rm-avail-dot"></span>Available Now</div></td>
//                           <td className="rm-td rm-td--cta"></td>
//                         </tr>
//                       ))}
//                     </tbody>
//                   </table>
...
//                   {[
//                     ...(property.parsedBedrooms || []).map(r => ({ ...r, _cat: 'Bedroom', _bookable: true })),
//                     ...(property.parsedBathrooms || []).map(r => ({ ...r, _cat: 'Bathroom', _bookable: false }))
//                   ].map((item, i) => {
//                     const price = Number(item.price) || Number(property.price) || 0;
//                     return (
...
//                                 {item.bedType && <span className="rm-tag">{item.bedType}</span>}
//                                 {item.ac && <span className="rm-tag rm-tag--ac">❄ AC</span>}
//                               </div>
//                             </div>
//                           </div>
//                           <div className="rm-mc-price">
//                             {(() => {
//                               const nightlyP = Number(item.price) || Number(property.meta?.perNightPrice) || Number(property.price) || 0;
//                               // Monthly price: property-level se lo (same jo booking card dikhata hai)
//                               const monthlyP = Number(property.meta?.perMonthPrice) || Number(property.meta?.monthlyPrice) || Number(property.monthly_price) || Number(property.price) || 0;
//                               const displayP = pricingMode === 'monthly' ? monthlyP : nightlyP;
//                               return item._bookable && displayP > 0 ? (
//                                 <><span className="rm-mc-price-main">₹{displayP.toLocaleString('en-IN')}</span><span className="rm-mc-price-unit">/{pricingMode === 'monthly' ? 'month' : 'night'}</span></>
//                               ) : item._bookable ? <span className="rm-on-request">On Request</span> : <span className="rm-on-request">Included</span>;
//                             })()}
//                           </div>
//                         </div>
//                         <div className="rm-mc-grid">
//                           <div className="rm-mc-cell">
//                             <span className="rm-mc-label">Type</span>
//                             <div className={`rm-bath ${item._cat === 'Bedroom' ? 'rm-bath--attached' : 'rm-bath--shared'}`}>
//                               <span className="rm-bath-dot"></span><span>{item._cat}</span>
//                             </div>
//                           </div>
//                           <div className="rm-mc-cell">
//                             <span className="rm-mc-label">Area</span>
//                             <span className="rm-area-val">{item.areaSqFt ? `${item.areaSqFt} sqft` : (property.area ? `${property.area} sqft` : '—')}</span>
//                           </div>
//                         </div>
//                         {item._bookable && (
//                           <button className="rm-mc-enquire rm-mc-book-btn" onClick={() => handleRoomBookNow(item)}>
//                             Book Now
//                           </button>
//                         )}
//                       </div>
...
//           {/* ================================================================
//               PG ROOM TABLE — EXISTING FORMAT + BOOK NOW BUTTON ADDED
//               ================================================================ */}
//           {property.parsedBedrooms?.length > 0 && property.property_category === 'PG' && pricingMode === 'monthly' && (
//             <>
//               <div className="divider"></div>
//               <div className="text-section">
//                 <div className="rm-section-header">
...
//                       <strong>
//                         ₹{pricingMode === 'daily'
//                           ? (Number(property.meta?.perNightPrice) || Math.min(...property.parsedBedrooms.map(r => Number(r.price) || Infinity).filter(p => p < Infinity))).toLocaleString('en-IN')
//                           : Math.min(...property.parsedBedrooms.map(r => Number(r.price) || Infinity).filter(p => p < Infinity)).toLocaleString('en-IN')
//                         }
//                       </strong>/{pricingMode === 'daily' ? 'night' : 'month'}
//                     </p>
//                   </div>
//                   <div className="rm-section-stats">
...
//                         <th className="rm-th">Bathroom</th>
//                         <th className="rm-th">Area</th>
//                         <th className="rm-th rm-th--price">Price / {pricingMode === 'daily' ? 'Night' : 'Month'}</th>
//                         <th className="rm-th">Available</th>
//                         <th className="rm-th rm-th--action"></th>
//                       </tr>
//                     </thead>
//                     <tbody>
//                       {property.parsedBedrooms.map((room, i) => {
//                         // nightly: use perNightPrice from meta, fallback to room.price
//                         // monthly: use room.price (that's monthly price from PG form)
//                         const nightlyPrice = Number(property.meta?.perNightPrice) || Number(room.perNightPrice) || Number(room.price) || 0;
//                         const monthlyPrice = Number(room.price) || 0;
//                         const displayPrice = pricingMode === 'daily' ? nightlyPrice : monthlyPrice;
//                         const priceUnit = pricingMode === 'daily' ? '/night' : '/mo';
...
//                                   </div>
//                                   {pricingMode === 'monthly' && room.securityDeposit && <div className="rm-deposit">Dep: ₹{Number(room.securityDeposit).toLocaleString('en-IN')}</div>}
//                                 </div>
//                               ) : <span className="rm-on-request">On Request</span>}
//                             </td>
//                             <td className="rm-td">
...
//                           <div className="rm-mc-price">
//                             {displayPrice > 0 ? (
//                               <><span className="rm-mc-price-main">₹{displayPrice.toLocaleString('en-IN')}</span><span className="rm-mc-price-unit">{priceUnit}</span></>
//                             ) : <span className="rm-on-request">On Request</span>}
//                           </div>
//                         </div>
//                         <div className="rm-mc-grid">
...
//                           </div>
//                         )}
//                         {/* PG Mobile — Both Enquire + Book Now */}
//                         <div style={{ display: 'flex', gap: '8px', marginTop: '4px' }}>
//                           <button
//                             className="rm-mc-enquire"
//                             style={{ flex: 1, background: '#f1f5f9', color: '#475569' }}
//                             onClick={() => { setSelectedRoomForLead(room.type); setShowLeadModal(true); }}
//                           >
//                             Enquire
//                           </button>
//                           <button
//                             className="rm-mc-enquire rm-mc-book-btn"
//                             style={{ flex: 1 }}
//                             onClick={() => handleRoomBookNow(room)}
//                           >
//                             Book Now
//                           </button>
//                         </div>
...
//                 <div className="rule-icon">{(property.smoking_allowed || property.smokingAllowed || property.meta?.smokingAllowed) ? <FiCheck className="text-green" /> : <FiXCircle className="text-red" />}</div>
//                 <div className="rule-info"><span className="rule-label">Smoking</span><strong className={(property.smoking_allowed || property.smokingAllowed || property.meta?.smokingAllowed) ? "text-green" : "text-gray"}>{(property.smoking_allowed || property.smokingAllowed || property.meta?.smokingAllowed) ? 'Allowed' : 'Not allowed'}</strong></div>
//               </div>
//               <div className="amenity-card rule-card">
//                 <div className="rule-icon">{(property.pets_allowed || property.petsAllowed || property.meta?.petsAllowed) ? <FiCheck className="text-green" /> : <FiXCircle className="text-red" />}</div>
//                 <div className="rule-info"><span className="rule-label">Pets</span><strong className={(property.pets_allowed || property.petsAllowed || property.meta?.petsAllowed) ? "text-green" : "text-gray"}>{(property.pets_allowed || property.petsAllowed || property.meta?.petsAllowed) ? 'Allowed' : 'Not allowed'}</strong></div>
//               </div>
//               <div className="amenity-card rule-card">
//                 <div className="rule-icon">{(property.events_allowed || property.eventsAllowed || property.meta?.eventsAllowed) ? <FiCheck className="text-green" /> : <FiXCircle className="text-red" />}</div>
//                 <div className="rule-info"><span className="rule-label">Events</span><strong className={(property.events_allowed || property.eventsAllowed || property.meta?.eventsAllowed) ? "text-green" : "text-gray"}>{(property.events_allowed || property.eventsAllowed || property.meta?.eventsAllowed) ? 'Allowed' : 'Not allowed'}</strong></div>
//               </div>
//               <div className="amenity-card rule-card">
//                 <div className="rule-icon">{(property.drinking_allowed || property.drinkingAllowed || property.meta?.drinkingAllowed) ? <FiCheck className="text-green" /> : <FiXCircle className="text-red" />}</div>
//                 <div className="rule-info"><span className="rule-label">Alcohol</span><strong className={(property.drinking_allowed || property.drinkingAllowed || property.meta?.drinkingAllowed) ? "text-green" : "text-gray"}>{(property.drinking_allowed || property.drinkingAllowed || property.meta?.drinkingAllowed) ? 'Allowed' : 'Not allowed'}</strong></div>
//               </div>
//               <div className="amenity-card rule-card">
//                 <div className="rule-icon">{(property.outside_guests_allowed || property.outsideGuestsAllowed || property.meta?.outsideGuestsAllowed) ? <FiCheck className="text-green" /> : <FiXCircle className="text-red" />}</div>
//                 <div className="rule-info"><span className="rule-label">Outside Guests</span><strong className={(property.outside_guests_allowed || property.outsideGuestsAllowed || property.meta?.outsideGuestsAllowed) ? "text-green" : "text-gray"}>{(property.outside_guests_allowed || property.outsideGuestsAllowed || property.meta?.outsideGuestsAllowed) ? 'Allowed' : 'Not allowed'}</strong></div>
//               </div>
//               <div className="amenity-card rule-card">
//                 <div className="rule-icon">{guestPolicy.family_allowed ? <FiCheck className="text-green" /> : <FiXCircle className="text-red" />}</div>
//                 <div className="rule-info"><span className="rule-label">Family</span><strong className={guestPolicy.family_allowed ? "text-green" : "text-gray"}>{guestPolicy.family_allowed ? 'Allowed' : 'Not allowed'}</strong></div>
//               </div>
//               <div className="amenity-card rule-card">
//                 <div className="rule-icon">{guestPolicy.unmarried_couple_allowed ? <FiCheck className="text-green" /> : <FiXCircle className="text-red" />}</div>
//                 <div className="rule-info"><span className="rule-label">Unmarried Couples</span><strong className={guestPolicy.unmarried_couple_allowed ? "text-green" : "text-gray"}>{guestPolicy.unmarried_couple_allowed ? 'Allowed' : 'Not allowed'}</strong></div>
//               </div>
//               <div className="amenity-card rule-card">
//                 <div className="rule-icon">{(guestPolicy.bachelors_allowed || guestPolicy.Bechelors) ? <FiCheck className="text-green" /> : <FiXCircle className="text-red" />}</div>
//                 <div className="rule-info"><span className="rule-label">Bachelor</span><strong className={(guestPolicy.bachelors_allowed || guestPolicy.Bechelors) ? "text-green" : "text-gray"}>{(guestPolicy.bachelors_allowed || guestPolicy.Bechelors) ? "Allowed" : "Not allowed"}</strong></div>
//               </div>
//               {property.houseRules?.includes("Late Entry Allowed") && (<div className="amenity-card rule-card"><div className="rule-icon"><FiCheck className="text-green" /></div><div className="rule-info"><span className="rule-label">Late Entry</span><strong className="text-green">Allowed</strong></div></div>)}
//               {property.houseRules?.includes("Friends Allowed") && (<div className="amenity-card rule-card"><div className="rule-icon"><FiCheck className="text-green" /></div><div className="rule-info"><span className="rule-label">Friends Entry</span><strong className="text-green">Allowed</strong></div></div>)}
//               {property.houseRules?.includes("Veg Only") && (<div className="amenity-card rule-card"><div className="rule-icon"><UtensilsCrossed size={18} className="text-green" /></div><div className="rule-info"><span className="rule-label">Food Preference</span><strong className="text-green">Pure Veg Only</strong></div></div>)}
//               {property.houseRules?.includes("Girlfriend/Boyfriend Entry Allowed") && (<div className="amenity-card rule-card"><div className="rule-icon"><FiCheck className="text-green" /></div><div className="rule-info"><span className="rule-label">Partner Entry</span><strong className="text-green">Allowed</strong></div></div>)}
...
//               {(property.lockInPeriod || property.meta?.lockInPeriod) && (<div className="amenity-card rule-card"><div className="rule-icon"><FiLock style={{ color: '#3b82f6' }} /></div><div className="rule-info"><span className="rule-label">Lock-in Period</span><strong>{property.lockInPeriod || property.meta?.lockInPeriod} Months</strong></div></div>)}
...
//               {(property.waterCharges || property.meta?.waterCharges) && (<div className="amenity-card rule-card"><div className="rule-icon"><Bus size={18} color="#0ea5e9" /></div><div className="rule-info"><span className="rule-label">Water Bill</span><strong>{property.waterCharges || property.meta?.waterCharges}</strong></div></div>)}
//               {(property.gateClosingTime || property.meta?.gateClosingTime || (property.property_category === 'PG' && property.check_in_time)) && (<div className="amenity-card rule-card"><div className="rule-icon"><Clock size={18} color="#ef4444" /></div><div className="rule-info"><span className="rule-label">Gate Closing</span><strong>{property.gateClosingTime || property.meta?.gateClosingTime || property.check_in_time}</strong></div></div>)}
//               {(property.foodAvailable !== undefined || property.meta?.foodAvailable !== undefined) && (
//                 <div className="amenity-card rule-card">
//                   <div className="rule-icon">{property.foodAvailable || property.meta?.foodAvailable ? <UtensilsCrossed size={18} className="text-green" /> : <FiXCircle className="text-red" />}</div>
...
//                       <div style={{ fontSize: '0.75rem', color: '#64748b' }}>{[property.foodDetails.breakfast && 'Breakfast', property.foodDetails.lunch && 'Lunch', property.foodDetails.dinner && 'Dinner'].filter(Boolean).join(', ')}</div>
//                     )}
//                   </div>
//                 </div>
//               )}
...
//                 <span style={{ color: '#475569' }}>{property.cancellation_policy}{' '}
//                   <a href="/refund-cancellation-policy" target="_blank" rel="noopener noreferrer" style={{ marginLeft: '6px', color: '#8b0000', fontWeight: '600', textDecoration: 'underline' }}>Read full policy</a>
//                 </span>
//               </div>
//             )}
//           </div>
...
//                     <p className="gbSubTitle">Helpful local information for your stay (transport, dining, essentials, places).</p>
//                   </div>
//                 </div>
...
//                         {property.guidebook.transport_tips.taxi && (<div className="gbRow"><div className="gbRowLeft"><MapPinIcon size={16} className="gbRowIcon" /><span className="gbRowLabel">Taxi / Cab</span></div><div className="gbRowValue">{property.guidebook.transport_tips.taxi}</div></div>)}
//                         {property.guidebook.transport_tips.parking && (<div className="gbRow"><div className="gbRowLeft"><ParkingCircle size={16} className="gbRowIcon" /><span className="gbRowLabel">Parking</span></div><div className="gbRowValue">{property.guidebook.transport_tips.parking}</div></div>)}
//                         {property.guidebook.transport_tips.local_travel && (<div className="gbRow"><div className="gbRowLeft"><Bus size={16} className="gbRowIcon" /><span className="gbRowLabel">Local Travel</span></div><div className="gbRowValue">{property.guidebook.transport_tips.local_travel}</div></div>)}
//                         {property.guidebook.transport_tips.metro && (<div className="gbRow"><div className="gbRowLeft"><Train size={16} className="gbRowIcon" /><span className="gbRowLabel">Metro Station</span></div><div className="gbRowValue">{property.guidebook.transport_tips.metro}</div></div>)}
...
//                       <div className="gbCardHeader"><div className="gbIconWrap"><UtensilsCrossed size={18} /></div><div className="gbCardHeaderText"><div className="gbCardTitle">Cafes & Restaurants</div><div className="gbCardMeta">{property.guidebook.cafes_restaurants.length} nearby options</div></div></div>
//                       <div className="gbTableWrap">
//                         <table className="gbTable">
//                           <thead><tr><th>Name</th><th className="gbThRight">Distance</th></tr></thead>
//                           <tbody>{property.guidebook.cafes_restaurants.map((item, idx) => (<tr key={idx}><td className="gbTdName">{item?.name || '-'}</td><td className="gbTdRight">{typeof item?.distance_m === 'number' ? `${item.distance_m} m` : (item?.distance_m ?? '-')}</td></tr>))}</tbody>
//                         </table>
//                       </div>
//                     </div>
//                   )}
...
//                     <div className="gbCard">
//                       <div className="gbCardHeader"><div className="gbIconWrap"><ShoppingBasket size={18} /></div><div className="gbCardHeaderText"><div className="gbCardTitle">Essentials Nearby</div><div className="gbCardMeta">Daily needs & services</div></div></div>
//                       <div className="gbRows">
//                         {property.guidebook.essentials_nearby.atm && (<div className="gbRow"><div className="gbRowLeft"><Landmark size={16} className="gbRowIcon" /><span className="gbRowLabel">ATM</span></div><div className="gbRowValue">{property.guidebook.essentials_nearby.atm}</div></div>)}
...
//                         {property.guidebook.essentials_nearby.pharmacy && (<div className="gbRow"><div className="gbRowLeft"><Pill size={16} className="gbRowIcon" /><span className="gbRowLabel">Pharmacy</span></div><div className="gbRowValue">{property.guidebook.essentials_nearby.pharmacy}</div></div>)}
//                         {property.guidebook.essentials_nearby.gym && (<div className="gbRow"><div className="gbRowLeft"><Dumbbell size={16} className="gbRowIcon" /><span className="gbRowLabel">Gym / Fitness</span></div><div className="gbRowValue">{property.guidebook.essentials_nearby.gym}</div></div>)}
//                       </div>
//                     </div>
//                   )}
//                   {Array.isArray(property.guidebook?.must_visit_places) && property.guidebook.must_visit_places.length > 0 && (
//                     <div className="gbCard">
//                       <div className="gbCardHeader"><div className="gbIconWrap"><MapPinIcon size={18} /></div><div className="gbCardHeaderText"><div className="gbCardTitle">Must-Visit Places</div><div className="gbCardMeta">{property.guidebook.must_visit_places.length} recommendations</div></div></div>
//                       <div className="gbList">
//                         {property.guidebook.must_visit_places.map((p, idx) => (
//                           <div className="gbListItem" key={idx}>
//                             <div className="gbListItemTop"><span className="gbListItemTitle">{p?.place || '-'}</span></div>
//                             {p?.best_time && <div className="gbListItemMeta"><Clock size={14} /><span>Best time: {p.best_time}</span></div>}
//                           </div>
//                         ))}
//                       </div>
//                     </div>
...
//               BOOKING CARD
//               Rules:
//               1. PG + monthly  → hide entirely (table upar kaam kar rahi hai)
//               2. PG + nightly  → show nightly price, no toggle
//               3. Non-PG + nightly → show nightly price, no toggle  
//               4. Non-PG + monthly → show monthly price, no toggle (table se price)
//               ================================================================ */}
//           {!(property.property_category === 'PG' && pricingMode === 'monthly') && (
...
//               {/* NO TOGGLE — pricingMode is already set from list page, no need to show toggle here */}
...
//                     </div>
//                     {/* Non-PG: hint to select room from table */}
//                     {property.property_category !== 'PG' && property.parsedBedrooms?.length > 0 && (
//                       <span style={{ fontSize: '0.75rem', color: '#64748b', marginTop: '2px' }}>
//                         {selectedPrice
//                           ? `Selected: ₹${formatCurrency(selectedPrice)}/${pricingMode === 'monthly' ? 'month' : 'night'}`
...
//               {/* Nightly stay info — only for nightly mode */}
//               {pricingMode === 'daily' && (
...
//                   <strong>Nightly Stay:</strong> Pay only for the nights you actually stay. Perfect for travelers, exam candidates, or short business trips.
//                 </div>
//               )}
...
//                     <span>{pricingMode === 'monthly' ? `${property.noticePeriod || property.meta?.noticePeriod || 30} Days` : (formData.checkOutDate ? new Date(formData.checkOutDate).toLocaleDateString() : (property.check_out_time || 'Select Date'))}</span>
//                   </div>
//                 </div>
...
//                         return (
//                           <div key={idx} onClick={() => rPrice > 0 && setSelectedPrice(rPrice)} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.9rem', padding: '8px', borderRadius: '6px', cursor: rPrice > 0 ? 'pointer' : 'default', background: isSelected ? '#fef2f2' : 'transparent', border: isSelected ? '1px solid #8b0000' : '1px solid transparent' }}>
//                             <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
//                               <div style={{ width: '16px', height: '16px', borderRadius: '50%', border: isSelected ? '5px solid #8b0000' : '2px solid #cbd5e1', background: 'white' }}></div>
//                               <span style={{ color: isSelected ? '#8b0000' : '#64748b', fontWeight: isSelected ? 600 : 400 }}>{room.type} <span style={{ fontSize: '0.8rem' }}>({room.washroomType})</span></span>
//                             </div>
//                             <span style={{ fontWeight: 600, color: '#0f172a' }}>{rPrice ? `₹${formatCurrency(rPrice)}/mo` : 'N/A'}</span>
//                           </div>
//                         );
//                       })}
//                     </div>
...
//             </div>
//           </div>
//           )} {/* end: hide booking card for PG monthly */}
...
// ─── MOBILE ROOM TABLE HELPER ───────────────────────────────────────────────
// Matches desktop: Room | Bathroom | Area | Price | Available | Book
const MobileRoomTable = ({ bedrooms, pricingMode, property, onBookNow, onEnquire, showEnquire }) => {
  const rows = bedrooms || [];
          ...
          {rows.map((room, i) => {
            ...
            return (
              <tr key={i} className={`rm-mob-row${isLast ? ' rm-mob-row--last' : ''}`}>
                ...
                {/* Col 3: Area */}
                <td className="rm-mob-td rm-mob-td--area">
                  <span className="rm-mob-area">{area}</span>
                </td>
                {/* Col 4: Price */}
                ...
                <td className="rm-mob-td">
                  {room.availabilityDate ? (
                    ...
                    <span className="rm-mob-avail rm-mob-avail--now">
                      <span className="rm-mob-avail-dot"></span>Now
                    </span>
                  )}
                </td>
                {/* Col 6: Book / Enquire */}
                <td className="rm-mob-td rm-mob-td--action">
                  <div className="rm-mob-btns">
                    {showEnquire && (
                      <button className="rm-mob-btn rm-mob-btn--enq" onClick={() => onEnquire && onEnquire(room)}>Enq.</button>
                    )}
      ...
      <div className="content-grid">
        <div className="details-column">
          ...
          {(property.parsedBedrooms?.length > 0 || property.parsedBathrooms?.length > 0) && property.property_category !== 'PG' && (
            <>
              ...
              <div className="text-section">
                ...
                {/* Desktop Table — 1 bedroom = 1 row, bathroom info is a column in same row */}
                <div className="rm-table-outer">
                  <table className="rm-table">
                    <thead>
                      <tr>
                      ...
                      {property.parsedBedrooms?.map((room, i) => {
                        ...
                        return (
                          <tr key={`bed-${i}`} className={`rm-row ${isLast ? 'rm-row--last' : ''}`}>
                            ...
                            {/* Col 2: Bathroom type — column of this bedroom row */}
                            <td className="rm-td">
                              <div className={`rm-bath ${room.attachedBathroom ? 'rm-bath--attached' : 'rm-bath--shared'}`}>
                                <span className="rm-bath-dot"></span>
                                <span>{room.attachedBathroom ? 'Attached' : 'Shared'}</span>
                ...
                {/* ── MOBILE COMPACT TABLE — 1 bedroom = 1 row, no separate bathroom rows ── */}
                <MobileRoomTable
          ...
          <div className="text-section">
            <h3>House Rules & Policies</h3>
            <div className="amenities-grid">
              <div className="amenity-card rule-card"><div className="rule-icon">{(property.smoking_allowed || property.smokingAllowed || property.meta?.smokingAllowed) ? <FiCheck className="text-green" /> : <FiXCircle className="text-red" />}</div><div className="rule-info"><span className="rule-label">Smoking</span><strong className={(property.smoking_allowed || property.smokingAllowed || property.meta?.smokingAllowed) ? "text-green" : "text-gray"}>{(property.smoking_allowed || property.smokingAllowed || property.meta?.smokingAllowed) ? 'Allowed' : 'Not allowed'}</strong></div></div>
              <div className="amenity-card rule-card"><div className="rule-icon">{(property.pets_allowed || property.petsAllowed || property.meta?.petsAllowed) ? <FiCheck className="text-green" /> : <FiXCircle className="text-red" />}</div><div className="rule-info"><span className="rule-label">Pets</span><strong className={(property.pets_allowed || property.petsAllowed || property.meta?.petsAllowed) ? "text-green" : "text-gray"}>{(property.pets_allowed || property.petsAllowed || property.meta?.petsAllowed) ? 'Allowed' : 'Not allowed'}</strong></div></div>
              <div className="amenity-card rule-card"><div className="rule-icon">{(property.events_allowed || property.eventsAllowed || property.meta?.eventsAllowed) ? <FiCheck className="text-green" /> : <FiXCircle className="text-red" />}</div><div className="rule-info"><span className="rule-label">Events</span><strong className={(property.events_allowed || property.eventsAllowed || property.meta?.eventsAllowed) ? "text-green" : "text-gray"}>{(property.events_allowed || property.eventsAllowed || property.meta?.eventsAllowed) ? 'Allowed' : 'Not allowed'}</strong></div></div>
              <div className="amenity-card rule-card"><div className="rule-icon">{(property.drinking_allowed || property.drinkingAllowed || property.meta?.drinkingAllowed) ? <FiCheck className="text-green" /> : <FiXCircle className="text-red" />}</div><div className="rule-info"><span className="rule-label">Alcohol</span><strong className={(property.drinking_allowed || property.drinkingAllowed || property.meta?.drinkingAllowed) ? "text-green" : "text-gray"}>{(property.drinking_allowed || property.drinkingAllowed || property.meta?.drinkingAllowed) ? 'Allowed' : 'Not allowed'}</strong></div></div>
              <div className="amenity-card rule-card"><div className="rule-icon">{(property.outside_guests_allowed || property.outsideGuestsAllowed || property.meta?.outsideGuestsAllowed) ? <FiCheck className="text-green" /> : <FiXCircle className="text-red" />}</div><div className="rule-info"><span className="rule-label">Outside Guests</span><strong className={(property.outside_guests_allowed || property.outsideGuestsAllowed || property.meta?.outsideGuestsAllowed) ? "text-green" : "text-gray"}>{(property.outside_guests_allowed || property.outsideGuestsAllowed || property.meta?.outsideGuestsAllowed) ? 'Allowed' : 'Not allowed'}</strong></div></div>
              <div className="amenity-card rule-card"><div className="rule-icon">{guestPolicy.family_allowed ? <FiCheck className="text-green" /> : <FiXCircle className="text-red" />}</div><div className="rule-info"><span className="rule-label">Family</span><strong className={guestPolicy.family_allowed ? "text-green" : "text-gray"}>{guestPolicy.family_allowed ? 'Allowed' : 'Not allowed'}</strong></div></div>
              <div className="amenity-card rule-card"><div className="rule-icon">{guestPolicy.unmarried_couple_allowed ? <FiCheck className="text-green" /> : <FiXCircle className="text-red" />}</div><div className="rule-info"><span className="rule-label">Unmarried Couples</span><strong className={guestPolicy.unmarried_couple_allowed ? "text-green" : "text-gray"}>{guestPolicy.unmarried_couple_allowed ? 'Allowed' : 'Not allowed'}</strong></div></div>
              <div className="amenity-card rule-card"><div className="rule-icon">{(guestPolicy.bachelors_allowed || guestPolicy.Bechelors) ? <FiCheck className="text-green" /> : <FiXCircle className="text-red" />}</div><div className="rule-info"><span className="rule-label">Bachelor</span><strong className={(guestPolicy.bachelors_allowed || guestPolicy.Bechelors) ? "text-green" : "text-gray"}>{(guestPolicy.bachelors_allowed || guestPolicy.Bechelors) ? "Allowed" : "Not allowed"}</strong></div></div>
              {property.houseRules?.includes("Late Entry Allowed") && (<div className="amenity-card rule-card"><div className="rule-icon"><FiCheck className="text-green" /></div><div className="rule-info"><span className="rule-label">Late Entry</span><strong className="text-green">Allowed</strong></div></div>)}
              {property.houseRules?.includes("Friends Allowed") && (<div className="amenity-card rule-card"><div className="rule-icon"><FiCheck className="text-green" /></div><div className="rule-info"><span className="rule-label">Friends Entry</span><strong className="text-green">Allowed</strong></div></div>)}
              {property.houseRules?.includes("Veg Only") && (<div className="amenity-card rule-card"><div className="rule-icon"><UtensilsCrossed size={18} className="text-green" /></div><div className="rule-info"><span className="rule-label">Food Preference</span><strong className="text-green">Pure Veg Only</strong></div></div>)}
              {property.houseRules?.includes("Girlfriend/Boyfriend Entry Allowed") && (<div className="amenity-card rule-card"><div className="rule-icon"><FiCheck className="text-green" /></div><div className="rule-info"><span className="rule-label">Partner Entry</span><strong className="text-green">Allowed</strong></div></div>)}
