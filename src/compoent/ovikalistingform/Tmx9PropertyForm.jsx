...
const Tmx9PropertyForm = () => {
  ...
  const validateForStep = (s) => {
    ...
    if (s === 1) {
      if (form.bedrooms === "" || form.bedrooms < 0)
        newErrors.bedrooms = "Bedrooms required (>= 0)";
      if (form.beds === "" || form.beds <= 0)
        newErrors.beds = "At least one bed required";
      if (form.bathrooms === "" || form.bathrooms <= 0)
        newErrors.bathrooms = "At least one bathroom required";
      if (!form.area?.trim()) {
        newErrors.area = "Area is required";
      } else if (form.area.length > 20) {
        newErrors.area = "Area should not exceed 20 characters";
    ...
    if (s === 2) {
      ...
      if (!form.checkOutTime)
        newErrors.checkOutTime = "Check-out time required";
    ...
    if (s === 4) {
      if (!form.baseRate && form.baseRate !== 0)
        newErrors.baseRate = "Base rate is required";
      if (form.baseRate && Number(form.baseRate) <= 0)
        newErrors.baseRate = "Rate must be positive";
      if (
        form.weeklyDiscountPct &&
        (form.weeklyDiscountPct < 0 || form.weeklyDiscountPct > 100)
      )
        newErrors.weeklyDiscountPct = "Discount must be between 0 and 100";
      if (!form.selfCheckIn?.trim())
        newErrors.selfCheckIn = "Provide self-check-in method or note";
    }

    if (s === 5) {
      if (idFiles.length === 0)
        newErrors.idFiles = "Please upload at least one ID / document";
    }
  ...
  const renderError = (field) =>
    errors[field] ? (
      <span className="tmx9pf-error">{errors[field]}</span>
    ) : null;
  ...
  const handleAmenityToggle = (amenity) => {
    setForm((s) => ({
      ...s,
      amenities: { ...s.amenities, [amenity]: !s.amenities[amenity] },
    }));
  };
  ...
  // ---------------- handleSubmit with single owner_id ----------------
  const handleSubmit = async (e) => {
    e.preventDefault();
    setIsSubmitting(true);

    // ✅ ensure user / owner id present
    ...
    try {
      ...
      // 🔐 VERY IMPORTANT: same owner_id for this user for all listings
      ...
      const meta = {
        ...
        country: form.country,
        ...(form.latitude !== "" &&
          form.latitude !== undefined && { latitude: Number(form.latitude) }),
        ...(form.longitude !== "" &&
        ...
        registrationNumber: form.registrationNumber,
        localCompliance: form.localCompliance,
        ownerId: ownerId, // also inside meta for reference
      };
      fd.append("meta", JSON.stringify(meta));

      photoPreviews.previews.forEach((p, i) => {
        if (p && p.file)
          fd.append("photos", p.file, p.name || `photo-${i}`);
      });
      ...
      const response = await fetch(
        "https://townmanor.ai/api/ovika/properties/upload",
        {
          method: "POST",
          body: fd,
          credentials: "include",
  ...
  const Toggle = ({ name, value, onChange }) => (
    <div className="tmx9pf-toggle" role="group" aria-label={name}>
      <button
        type="button"
        className={`option ${value === false ? "active" : ""}`}
      ...
      <button
        type="button"
        className={`option ${value === true ? "active" : ""}`}
  ...
  // ---------------- JSX (steps UI) – same as your original ----------------
  return (
    <form
      className="tmx9pf-root tmx9pf-paginated"
      onSubmit={handleSubmit}
      noValidate
    >
      <header className="tmx9pf-header">
        <h1 className="tmx9pf-title">
          Create <span className="span-data-setup">Property</span> Listing
        </h1>
        <p className="tmx9pf-sub">
          Fill the sections below carefully. Use Next / Previous to navigate
          steps.
        </p>
      ...
      <div className="tmx9pf-card">
        ...
        {step === 0 && (
          <section className="tmx9pf-section">
            <h2 className="tmx9pf-section-title">
              Basic Property Information &amp; Description
            </h2>
            <div className="tmx9pf-grid">
              <div className="tmx9pf-field">
                <label className="tmx9pf-label">Property Type</label>
              ...
              <div className="tmx9pf-field">
                ...
                <select
                  ...
                  className="tmx9pf-select"
                >
                  {DEFAULT_PROPERTY_CATEGORIES.map((c) => (
              ...
              <div className="tmx9pf-field full">
                ...
                <input
                  ...
                  className={`tmx9pf-input ${
                    errors.title ? "tmx9pf-input--error" : ""
                  }`}
              ...
              <div className="tmx9pf-field full">
                ...
                <textarea
                  ...
                  onChange={handleChange}
                  rows="4"
                  className={`tmx9pf-textarea ${
                    errors.mainDescription ? "tmx9pf-input--error" : ""
                  }`}
                />
                {renderError("mainDescription")}
              </div>
              ...
              <div className="tmx9pf-field full">
                ...
                <input
                  ...
                  onChange={handleChange}
                  className={`tmx9pf-input ${
                    errors.address ? "tmx9pf-input--error" : ""
                  }`}
                />
                {renderError("address")}
              </div>
              ...
              <div className="tmx9pf-field">
                ...
                <input
                  ...
                  onChange={handleChange}
                  className={`tmx9pf-input ${
                    errors.country ? "tmx9pf-input--error" : ""
                  }`}
                />
                {renderError("country")}
              </div>
  ...
  onChange={(e) => {
    // sirf digits allow karo, max 10 char
...
/>
              ...
              <div className="tmx9pf-field">
                ...
                <input
                  ...
                  onChange={handleChange}
                  className={`tmx9pf-input ${
                    errors.city ? "tmx9pf-input--error" : ""
                  }`}
                />
                {renderError("city")}
              </div>
              ...
              <div className="tmx9pf-field">
                ...
                <input
                  ...
                  onChange={handleNumChange}
                  className={`tmx9pf-input ${
                    errors.latitude ? "tmx9pf-input--error" : ""
                  }`}
                  placeholder="e.g., 19.0760"
                />
                {renderError("latitude")}
              ...
              <div className="tmx9pf-field">
                ...
                <input
                  ...
                  onChange={handleNumChange}
                  className={`tmx9pf-input ${
                    errors.longitude ? "tmx9pf-input--error" : ""
                  }`}
                  placeholder="e.g., 72.8777"
                />
                {renderError("longitude")}
            ...
            <div className="tmx9pf-grid">
              <div className="tmx9pf-field">
                ...
                <input
                  ...
                  onChange={handleNumChange}
                  className={`tmx9pf-input ${
                    errors.bedrooms ? "tmx9pf-input--error" : ""
                  }`}
                />
                {renderError("bedrooms")}
              </div>
              ...
              <div className="tmx9pf-field">
                ...
                <input
                  ...
                  onChange={handleNumChange}
                  className={`tmx9pf-input ${
                    errors.beds ? "tmx9pf-input--error" : ""
                  }`}
                />
                {renderError("beds")}
              </div>
              ...
              <div className="tmx9pf-field">
                ...
                <input
                  ...
                  onChange={handleNumChange}
                  className={`tmx9pf-input ${
                    errors.bathrooms ? "tmx9pf-input--error" : ""
                  }`}
                />
                {renderError("bathrooms")}
              </div>
              ...
              <div className="tmx9pf-field">
                ...
                <input
                  ...
                  maxLength={20}
                  className={`tmx9pf-input ${
                    errors.area ? "tmx9pf-input--error" : ""
                  }`}
                  placeholder="e.g., 1500 sqft"
                />
                {renderError("area")}
              ...
              <div className="tmx9pf-field full">
                <label className="tmx9pf-label">
                  Amenities (select all that apply)
                </label>
                <div className="tmx9pf-amenities">
                  {Object.entries(AMENITIES).map(([group, list]) => (
                    <div key={group} className="tmx9pf-amenity-group">
                      <div className="tmx9pf-amenity-group-title">
                        {group}
                      </div>
                      <div className="tmx9pf-amenity-list">
                        {list.map((a) => (
            ...
            <div className="tmx9pf-grid">
              <div className="tmx9pf-field">
                ...
                <input
                  ...
                  onChange={handleChange}
                  className={`tmx9pf-input ${
                    errors.checkInTime ? "tmx9pf-input--error" : ""
                  }`}
                />
                {renderError("checkInTime")}
              </div>
              ...
              <div className="tmx9pf-field">
                ...
                <input
                  ...
                  onChange={handleChange}
                  className={`tmx9pf-input ${
                    errors.checkOutTime ? "tmx9pf-input--error" : ""
                  }`}
                />
                {renderError("checkOutTime")}
              </div>
              ...
              <div className="tmx9pf-field">
                ...
                <input
                  ...
                  onChange={handleNumChange}
                  className={`tmx9pf-input ${
                    errors.maxGuests ? "tmx9pf-input--error" : ""
                  }`}
                />
                {renderError("maxGuests")}
              </div>
            ...
            <div className="tmx9pf-grid">
              <div className="tmx9pf-field full">
                ...
                <input
                  ...
                  className={`tmx9pf-file ${
                    errors.photos ? "tmx9pf-file--error" : ""
                  }`}
                />
                {renderError("photos")}
                <div className="tmx9pf-photo-previews">
                  {photoPreviews.previews.length === 0 && (
                    <div className="tmx9pf-muted">No photos selected</div>
                  )}
                  {photoPreviews.previews.map((p, i) => (
                    <div key={i} className="tmx9pf-photo-thumb">
                      <img src={p.url} alt={p.name} />
                      <div className="tmx9pf-photo-actions">
                        <button
                          ...
                          className={`tmx9pf-small-btn ${
                            coverIndex === i ? "tmx9pf-small-btn--active" : ""
                          }`}
                        >
                          Cover
                        </button>
            ...
            <div className="tmx9pf-grid">
              <div className="tmx9pf-field">
                ...
                <input
                  ...
                  onChange={handleNumChange}
                  className={`tmx9pf-input ${
                    errors.baseRate ? "tmx9pf-input--error" : ""
                  }`}
                />
                {renderError("baseRate")}
              </div>
              ...
              <div className="tmx9pf-field">
                ...
                <input
                  ...
                  onChange={handleNumChange}
                  className={`tmx9pf-input ${
                    errors.weeklyDiscountPct ? "tmx9pf-input--error" : ""
                  }`}
                />
                {renderError("weeklyDiscountPct")}
              </div>
              ...
              <div className="tmx9pf-field">
                ...
                <input
                  ...
                  onChange={handleChange}
                  className={`tmx9pf-input ${
                    errors.selfCheckIn ? "tmx9pf-input--error" : ""
                  }`}
                  placeholder="e.g., Smart lock, Lockbox"
                />
                {renderError("selfCheckIn")}
              </div>
            </div>
        ...
        {step === 5 && (
          <section className="tmx9pf-section">
            ...
            <div className="tmx9pf-grid">
              <div className="tmx9pf-field full">
                <label className="tmx9pf-label">
                  Government ID / Business documents *
                </label>
                <input
                  type="file"
                  accept=".pdf,image/*"
              ...
              <div className="tmx9pf-field">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setDamageProtection(e.target.value === "yes")
                  }
                  className="tmx9pf-select"
                >
                  <option value="no">No</option>
      ...
      <div className="tmx9pf-nav">
        <button
          ...
          onClick={goPrev}
          disabled={step === 0}
          className="tmx9pf-nav-btn tmx9pf-nav-prev"
        >
          Previous
        </button>
        {step < STEPS.length - 1 ? (
          <button
            type="button"
            onClick={goNext}
            className="tmx9pf-nav-btn tmx9pf-nav-next"
          >
            Next
          </button>
        ) : (
