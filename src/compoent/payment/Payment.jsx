...
// ========== CALENDAR API HELPERS (UPDATED WITH ERROR HANDLING) ==========
const CALENDAR_API_BASE = 'https://townmanor.ai/api/calendar'; // Updated to use production URL
...
async function getCalendar(propertyKey) {
  try {
    ...
    if (!res.ok) {
      ...
      return { blocked: [] }; // Return empty blocked dates if API fails
    }
    
    return await res.json();
  } catch (error) {
    console.warn('Failed to fetch calendar, using default data:', error);
    return { blocked: [] }; // Return empty blocked dates on error
  }
}
...
function Payment() {
  ...
  // Single API endpoint for passport verification
  ...
  // Find where the passport verification form should be rendered in your JSX
  // Look for the passport verification section in your JSX and update it with the following:
  /*
  <div className="passport-verification">
    <h3>Passport Verification</h3>
    <div className="form-group">
      <label htmlFor="fileNumber">Passport File Number</label>
      <input
        type="text"
        id="fileNumber"
        value={passportInput}
    ...
    <button
      ...
      disabled={isPassportLoading || !passportInput || !passportDob}
  ...
  */
  ...
  const handleVerifyPassport = async () => {
    ...
    try {
      // Single API call for passport verification
      const response = await axios.post(
        PASSPORT_VERIFY_URL,
        {
      ...
      if (response.data.success && response.data.data) {
        ...
        setFormData(prev => ({
          ...
          fileNumber: fileNumber  // Store the file number in form data
        }));
