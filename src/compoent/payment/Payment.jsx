...
// //         phone: userData.phone || formData.phoneNumber || '',
...
// //                 {/* Phone Number Input - No Verification Required */}
// //                 <div className="veri-card-1" style={{ marginBottom: '20px' }}>
// //                   <h3 className="veri-title-1">Contact Number</h3>
...
// //                       style={{ width: '100%' }}
// //                     />
// //                     <p style={{ marginTop: '8px', fontSize: '12px', color: '#4caf50' }}>
...
// //                     Passport verification — for foreign customers or where Aadhaar not available.
// //                   </div>
// //                 </div>
...
//       let userPhone = '9999999999'; // Default phone number
...
//       // Prepare booking details with all required fields
//       const bookingDetails = {
...
function Payment() {
  ...
  // NEW: passport file state for international passport verification
  ...
  const PASSPORT_VERIFY_URL = 'https://kyc-api.surepass.app/api/v1/ocr/international-passport-v2';
  const SUREPASS_HEADERS = {
    'Authorization': 'Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..DfipEQt4RqFBQbOK29jbQju3slpn0wF9aoccdmtIsPg',
    'Accept': 'application/json'
    // NOTE: do NOT set Content-Type here for multipart/form-data requests; axios will set it.
  };

  // NEW: handle passport file selection (for international passport verification)
  ...
  // NEW: handle passport verification by uploading the passport file to the provided API
  const handleVerifyPassport = async () => {
    setFormData(prev => ({ ...prev, passportVerified: false }));
    setPassportError('');
    ...
    try {
      ...
      // Common field name: 'file' — change this if your API expects a different field key.
      fd.append('file', passportFile);

      const response = await axios.post(PASSPORT_VERIFY_URL, fd, {
        headers: {
          ...SUREPASS_HEADERS,
          // IMPORTANT: do NOT set 'Content-Type' here; axios will set the multipart boundary.
        },
        timeout: 60000,
      });

      if (response.data && (response.data.success || response.status === 200)) {
        // store any returned data for later usage
  ...
  const handlePayNow = async () => {
    ...
    try {
      ...
      const bookingDetails = {
        ...
        username: userLocal.username || username || 'guest',
        phone_number: userPhone,
        aadhar_number: aadhaarNumber || (formData.passportVerified ? 'PASSPORT_VERIFIED' : 'NOT_PROVIDED'),
        user_photo: formData.uploadedPhoto || '',
        terms_verified: true,
        email: userEmail || 'guest@townmanor.ai',
      };
            ...
            {step === 4 && (
              <div>
                ...
                <div style={{ marginBottom: '12px' }}>
                  ...
                  <div style={{ fontSize: '12px', color: '#666', marginTop: '6px' }}>
                    Passport verification — for foreign customers or where Aadhaar not available. (International passport upload)
                  </div>
                </div>
                ...
                <div className="veri-container-1">
                  {verificationMethod === 'aadhaar' ? (
                    ...
                    // NEW UI: passport file upload + verify button
                    <div className="veri-card-1">
                      <h3 className="veri-title-1">International Passport Verification</h3>

                      <div className="mb-4">
                        <label className="veri-label-1 block mb-1">Upload Passport (photo / scan)</label>
                        <input
                          type="file"
