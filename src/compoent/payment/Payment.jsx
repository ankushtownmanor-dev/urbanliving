...
// ========== CALENDAR COMPONENT (UNCHANGED BEHAVIOR) ==========
const Calendar = ({ selectedDates, onDateSelect, minDate = new Date(), disabledDateSet = new Set(), onInvalidRange }) => {
  ...
  useEffect(() => {
    setCheckInDate(selectedDates.checkInDate ? new Date(selectedDates.checkInDate) : null);
    setCheckOutDate(selectedDates.checkOutDate ? new Date(selectedDates.checkOutDate) : null);
  }, [selectedDates]);
  ...
  const isDateDisabled = (date) => {
    ...
    const min = new Date(minDate.toDateString());
    return date < min || disabledDateSet.has(dateStr);
  };
...
function Payment() {
  ...
  // ⭐ EMAIL SENDING FUNCTION (FRONTEND via EmailJS)
  const sendBookingConfirmationEmail = async (bookingData) => {
    try {
      ...
      const nights = differenceInDays(
        new Date(formData.checkOutDate || new Date()),
        new Date(formData.checkInDate || new Date())
      );

      const emailParams = {
        to_email: userLocal.email || '',
        to_name: userLocal.username || 'Guest',
        property_name: property?.name || 'Property',
        check_in_date: formData.checkInDate ? format(new Date(formData.checkInDate), 'dd MMM yyyy') : '',
        check_out_date: formData.checkOutDate ? format(new Date(formData.checkOutDate), 'dd MMM yyyy') : '',
        total_amount: pricing.total ? Number(pricing.total).toFixed(2) : '0.00',
        subtotal: pricing.subtotal ? Number(pricing.subtotal).toFixed(2) : '0.00',
        gst: pricing.gst ? Number(pricing.gst).toFixed(2) : '0.00',
        booking_id: bookingData.bookingId || bookingData.booking_id || bookingData || 'N/A',
      ...
      await emailjs.send(
        'service_ggypt4s',      // service id (replace if needed)
        'template_irruvtk',     // template id (replace if needed)
        emailParams
      );

      console.log('Email sent successfully');
      showAlert('Booking confirmation email sent successfully!');
      return true;
    } catch (error) {
      console.error('Failed to send email:', error);
      showAlert('Booking confirmed but email failed to send. Please check your inbox or contact support.');
      return false;
    }
  };
  ...
  useEffect(() => {
    try {
      ...
      if (Array.isArray(savedOtpDigits) && savedOtpDigits.length === 6) {
        // not strictly needed but restored if present
      }
      if (savedAadhaar) {
  ...
  useEffect(() => { if (!hasLoadedFromStorageRef.current) return; localStorage.setItem(STORAGE_KEYS.step, String(step)); }, [step]);
  useEffect(() => { if (!hasLoadedFromStorageRef.current) return; try { localStorage.setItem(STORAGE_KEYS.formData, JSON.stringify(formData)); } catch {} }, [formData]);
  useEffect(() => { if (!hasLoadedFromStorageRef.current) return; try { localStorage.setItem(STORAGE_KEYS.phoneDigits, JSON.stringify(phoneDigits)); } catch {} }, [phoneDigits]);
  useEffect(() => { if (!hasLoadedFromStorageRef.current) return; try { localStorage.setItem(STORAGE_KEYS.otpDigits, JSON.stringify([])); } catch {} }, [/* no-deps intentionally to avoid constant writes */]);
  useEffect(() => { if (!hasLoadedFromStorageRef.current) return; localStorage.setItem(STORAGE_KEYS.aadhaar, aadhaarNumber); }, [aadhaarNumber]);
  ...
  const handleOtpKeyDown = (index, e) => {
    if (e.key === 'Backspace' && !/*otpDigits*/'' && index > 0) {
      // otpInputsRef.current[index - 1]?.focus(); // not using multiple tiny inputs here
    }
  };
  ...
  // ⭐ MODIFIED handleProceedToPayment - uses backend payu endpoint to return params then posts form
  const handleProceedToPayment = async (bookingIdParam) => {
    ...
    try {
      ...
      const amountToUse = pricing && pricing.total && Number(pricing.total) > 0 ? Number(pricing.total) : 1;

      const redirectSuccessUrl = `${window.location.origin}/payment?status=success`;
      const redirectFailureUrl = `${window.location.origin}/payment?status=failure`;

      const paymentData = {
        key: 'UvTrjC',
        txnid: txnid,
        amount: amountToUse, // actual total amount
        productinfo: 'Room Booking',
        firstname: userData.name || username || '',
        email: userData.email || '',
        phone: userData.phone || '',
        surl: `https://townmanor.ai/api/boster/payu/success?redirectUrl=${encodeURIComponent(redirectSuccessUrl)}`,
        furl: `https://townmanor.ai/api/boster/payu/failure?redirectUrl=${encodeURIComponent(redirectFailureUrl)}`,
        udf1: bookingIdParam || '',
  ...
  // ⭐ HANDLE SUCCESSFUL PAYMENT: send EmailJS then redirect to dashboard
  const handlePaymentSuccess = async () => {
    const bookingIdFromStorage = localStorage.getItem('bookingId') || bookingId;
    try {
      // attempt to send email (frontend via EmailJS)
      await sendBookingConfirmationEmail({ bookingId: bookingIdFromStorage });
    } catch (err) {
      console.error('Error while sending booking email after payment:', err);
    } finally {
      // redirect to dashboard (change path if your app uses different route)
      const finalBookingId = bookingIdFromStorage || '';
      window.location.href = `/dashboard?bookingId=${encodeURIComponent(finalBookingId)}`;
    }
  };

  // ⭐ CHECK FOR PAYMENT SUCCESS ON COMPONENT MOUNT (redirect from payment gateway)
  useEffect(() => {
    const urlParams = new URLSearchParams(window.location.search);
    const paymentStatus = urlParams.get('status') || urlParams.get('paymentStatus') || urlParams.get('statusCode');
    console.log('Payment status from URL:', paymentStatus);
    if (paymentStatus && (paymentStatus.toLowerCase() === 'success' || paymentStatus === '1' || paymentStatus === 'SUCCESS')) {
          ...
          <div className="progress-bar-container">
            {steps.map((stepName, index) => (
              <div key={index} className="progress-bar-step">
                ...
                <span className={`progress-bar-step-name ${index + 1 === step ? 'is-active' : ''}`}>
                  {stepName}
                </span>
              </div>
            ))}
                ...
                <div className="form-step-card">
                  ...
                  <div className="form-step-card-content">
                    ...
                    <p className="form-step-card-price">
                      <MdCurrencyRupee />{property?.per_night_price || 0}
                      <span className="form-step-card-price-per-night">/night</span>
                    </p>
                  </div>
                </div>
              </div>
                    ...
                    <div className="veri-card-1">
                      ...
                      <div className="veri-aadhaar-wrap-1">
                        ...
                        <button
                          ...
                          className={`veri-btn-1 ${formData.aadhaarVerified ? "veri-btn-verified-1" : "veri-btn-default-1"}`}
                        >
                    ...
                    <div className="veri-card-1">
                      ...
                      <div className="veri-aadhaar-wrap-1">
                        <input
                          ...
                          placeholder="Passport no. or client_id"
                        />
                        <button
                          onClick={handleVerifyPassport}
                          disabled={formData.passportVerified || !passportInput || isPassportLoading}
                          className={`veri-btn-1 ${formData.passportVerified ? "veri-btn-verified-1" : "veri-btn-default-1"}`}
                        >
